<?php
    ...
    class ImageHandler
    {
        //The folder in which to save images and maximum dimension
        //for the image
        public $save_dir;
        public $max_dims;
      ...
      /**
       *Resize/resamples an image uploaded via a web form
       *
       ...
       */
        public  function  processUploadedImage($file,$rename=TRUE)
        {
            ...
            //If an error occurred, throw an exeption
            ...
            //Rename file if flag is set to TRUE
            if($rename==TRUE)
            {
        ...
        /**
         * Determines the filetype adn extension of  an image
         *
         ...
         *
         * @param $img the path to the upload
         * @return array the new and original image dimension
         */
        private function getNewDims($img)
        {
            //Assemble the necessart variables for processing
            ...
            {
                //If the image is smaller then the max dimension, keep its
                // dimension be multiplying be  1
                $s = 1;
            }
         ...
         * what imade handling function should be used. This
        ...
        private function  getImageFunction($img)
        {
             ...
             *
             * Creates and saves a new image based on the new dimension
            ...
            private function doImageResize($img)
            {
               ...
               $d = $this->getNewDims($img);
                ...
                if(imagecopyresampled($new_img, $src_img, 0, 0, 0, 0, $d[0], $d[1], $d[2], $d[3]))
                {
                    ...
                    {
                        throw new Exception('Failed to  save  the new imdage!');
                    }
                }
                else
                {
